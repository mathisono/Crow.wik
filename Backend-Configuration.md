...
This page explains how Crow selects and configures bridge backends: Meshtastic UDP, Meshtastic TCP Port-API, MeshCore UDP, MeshCore TCP Companion API, and APRS backends.

Use this page when you are deciding **how Crow talks to each radio or transport**. Use [Configuring Channels](Configuring-Channels.md) when you are deciding **which message groups, keys, or channel mappings Crow should route**.
...
See also: [Meshtastic API Backend](Meshtastic-API.md).
...
After changing backend configuration, restart Crow:
...
7. No raw PSKs appear in logs.
8. Read-only discovery does not modify persistent config.
