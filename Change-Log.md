...
| Part 97 bridge policy | Needed clearer operator explanation. | Crow wiki now explains automatic forwarding risk and gateway responsibilities in plain language. | See Strict Gatekeeper Mode page. |
...
| [Command Reference](Command-Reference) | User-facing command list and syntax. | Added/updated. |
| [APRS Bridge](APRS) | APRS-IS, KISS TCP, APRS passcode, groups, Part 97-safe APRS behavior. | Added. |
| [Strict Gatekeeper Mode](Strict-Gatekeeper-Mode) | Fail-closed bridge filtering and Part 97 auto-forwarding explanation. | Expanded with tables and operator guidance. |
...
Strict Gatekeeper is present in Crow and is documented as a fail-closed bridge control.

It is intended to reduce risk when forwarding Meshtastic/MeshCore traffic into AREDN or other amateur-radio paths.

Key behaviors:

- drops encrypted Meshtastic packets before bridge forwarding;
- drops non-text bridged packets;
- requires a callsign-looking sender identity;
- optionally requires the sender to appear in `allowed_callsigns`;
- annotates accepted traffic as `[SENDER via GATEWAY] message`;
- gives operators a clearer control point for Part 97 automatic-forwarding concerns.
